...
namespace PALibrary.Library.DAO
{
    class AccountsDAO
    {
        ...
        public static DayBookInfo GetCashBookOpeningBalance(DateTime toDate)
        {
            ...
            DayBookInfo chitsOpening = GetChitsOpeniningBalance(toDate, DBConstant.CHITS_LEDGER, DBConstant.ACCOUNT_OPENING_CASH);
            ...
            DayBookInfo chitCommOpening = GetChitCommissionOpeningBalance(toDate, DBConstant.CHIT_COMMISSION_LEDGER, DBConstant.ACCOUNT_OPENING_CASH);
            if (chitCommOpening != null)
            {
                debit = debit + chitCommOpening.Credit;
                credit = credit + chitCommOpening.Debit;
            }
        ...
        public static List<DayBookInfo> GetTrailBalance(DateTime toDate)
        {
            ...
            DayBookInfo chitOpening = GetChitsOpeniningBalance(toDate, DBConstant.CHITS_LEDGER, DBConstant.ACCOUNT_OPENING);
            foreach (DayBookInfo tb in trialBalance)
            {
                if (tb.Particulars.Equals("CURRENT ASSETS"))
                {
        ...
        public static List<DayBookInfo> GetTrailBalanceDetails(DateTime toDate)
        {
            ...
            DayBookInfo chitOpening = GetChitsOpeniningBalance(toDate, DBConstant.CHITS_LEDGER, DBConstant.ACCOUNT_OPENING);
            detail = new DayBookInfo();
            detail.Particulars = DBConstant.CHITS_LEDGER;
            detail.Narration = "CURRENT ASSETS";
        ...
        public static DayBookInfo GetChitsOpeniningBalance(DateTime toDate, string ledgerName, int type)
        {
            ...
            //Chit commission 
            openingBalance = GetChitCommissionOpeningBalanceNoVoucher(toDate, DBConstant.CHIT_COMMISSION_LEDGER, type);
            if (openingBalance != null)
            {
                credit = credit + openingBalance.Credit;
                debit = debit + openingBalance.Debit;
        ...
        public static DayBookInfo GetChitCommissionOpeningBalanceNoVoucher(DateTime toDate, string ledgerName, int type)
        {
            decimal credit = 0;
            decimal debit = 0;
        ...
        public static DayBookInfo GetCompBiddingOpeningBalance(DateTime toDate, string ledgerName, int type)
        {
            ...
            DayBookInfo compOpenBalance = ChitsCompanyBiddingDAO.GetCompBiddingOpeningBalance(toDate, ledgerName, type);
            if (compOpenBalance != null)
            {
                credit = credit + compOpenBalance.Credit;
                debit = debit + compOpenBalance.Debit;
            }
